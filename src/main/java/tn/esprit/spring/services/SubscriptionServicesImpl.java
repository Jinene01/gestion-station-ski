...
public class SubscriptionServicesImpl implements ISubscriptionServices{
    ...
    public void showMonthlyRecurringRevenue() {
        Float monthly = getSafeRevenue(TypeSubscription.MONTHLY);
        Float semestriel = getSafeRevenue(TypeSubscription.SEMESTRIEL) / 6;
        Float annual = getSafeRevenue(TypeSubscription.ANNUAL) / 12;

        Float revenue = monthly + semestriel + annual;

        log.info("Monthly Revenue = " + revenue);
    }

    private Float getSafeRevenue(TypeSubscription typeSub) {
        Float result = subscriptionRepository.recurringRevenueByTypeSubEquals(typeSub);
